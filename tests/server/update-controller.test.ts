...
async function loadUpdateController(overrides: Partial<UpdateControllerMocks> = {}) {
  ...
  const readFileSync = overrides.readFileSync ?? vi.fn(() => JSON.stringify({
    name: 'hermes-web-ui',
    version: '0.0.0',
    repository: { url: 'https://github.com/IEatCodeDaily/hermes-studio.git' },
  }))
...
describe('update controller', () => {
  ...
  it('loads preview tags through async git with a short timeout', async () => {
    process.env.HERMES_WEB_UI_PREVIEW_REPO = 'https://github.com/IEatCodeDaily/hermes-studio'
    ...
    expect(mocks.execFile).toHaveBeenCalledWith(
      'git',
      ['ls-remote', '--tags', '--refs', 'https://github.com/IEatCodeDaily/hermes-studio.git'],
  ...
  it('falls back to GitHub API when async git tag loading fails', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://api.github.com/repos/IEatCodeDaily/hermes-studio/tags?per_page=100',
      expect.objectContaining({
